...
- **Data structures, not just strings**: values can be strings, hashes, lists, sets, sorted sets, etc. — used for caching, session storage, leaderboards, rate limiting, pub/sub messaging. As a cache, Redis is typically the "distributed cache" layer in [caching-fundamentals.md](caching-fundamentals.md); a hot key expiring under heavy concurrent traffic can trigger a [thundering herd / cache stampede](thundering-herd-problem.md) against the database it was protecting.
